...
** Last update Tue May 13 21:01:56 2008 caner candan
*/
...
int	act_up(t_info *info, t_client *client, char *param)
{
  ...
  if (client->direction == NORTH)
    ...
    client->x = (client->x == (info->x - 3) ? 0 : client->x + 1);
  else if (client->direction == WEST)
    client->x = (client->x == 0 ? info->x - 3 : client->x - 1);
  else
    client->y = (client->y == (info->y - 3) ? 0 : client->y + 1);
  return (0);
}
